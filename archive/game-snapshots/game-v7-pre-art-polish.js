...
class Snake {
    ...
    renderHead(ctx, x, y) {
        ...
        // Draw eye whites
        ctx.beginPath();
        ctx.arc(eye1X, eye1Y, GRID_SIZE * 0.12, 0, Math.PI * 2);
        ctx.arc(eye2X, eye2Y, GRID_SIZE * 0.12, 0, Math.PI * 2);
        ctx.fill();
        
        // Draw pupils
        ctx.fillStyle = '#000000';
        ctx.beginPath();
        ctx.arc(eye1X, eye1Y, GRID_SIZE * 0.06, 0, Math.PI * 2);
        ctx.arc(eye2X, eye2Y, GRID_SIZE * 0.06, 0, Math.PI * 2);
        ctx.fill();
        
        // Draw eye highlights
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.arc(eye1X - 1, eye1Y - 1, GRID_SIZE * 0.03, 0, Math.PI * 2);
        ctx.arc(eye2X - 1, eye2Y - 1, GRID_SIZE * 0.03, 0, Math.PI * 2);
        ctx.fill();
    ...
    renderBodySegment(ctx, x, y, index) {
        ...
        ctx.beginPath();
        ctx.arc(x + GRID_SIZE / 2, y + GRID_SIZE / 2, GRID_SIZE * 0.3, 0, Math.PI * 2);
        ctx.fill();
        ctx.globalAlpha = 1.0;
...
class Food {
    ...
    render(ctx) {
        // Draw food as energy crystal (hexagonal gem shape)
        ctx.save();
        ...
        gradient.addColorStop(0.4, '#ff6699');
        gradient.addColorStop(0.8, '#ff3366');
...
class PowerUp {
    ...
    render(ctx) {
        ...
        // Main power-up body - diamond shape with outline
        ctx.translate(centerX, centerY);
        ...
        gradient.addColorStop(0.4, this.type.color);
        ...
        // Reset rotation and draw icon text
        ctx.rotate(-Math.PI / 4);
        ctx.shadowBlur = 0;
        ctx.fillStyle = '#000000';
